...
## 1. Public API shape is currently very unstable / too exposed

Almost every module is exposed directly:
...
For an alpha release, that may be acceptable, but it means users can depend on every internal helper, query trait, and parsing utility. Changing any of those later becomes a breaking change.

### Consider

- Keep model modules public if desired.
- Hide implementation-oriented modules like:
    - `client`
    - `base_hardcover_item`
    - maybe `util`
- Re-export only the intended public types.
...
If `client::set_api_key` is intended public API, you could expose that directly:

```rust
pub use client::set_api_key;
```


instead of requiring users to know about the `client` module.

---

## 2. Global API key design will become limiting

The library stores the API key in a global `OnceLock<String>`.

Issues:

- It cannot be changed after first set.
- Multiple API keys in one process are impossible.
- Tests can interfere with each other.
- It makes the library harder to use in larger applications.
- It prevents per-request or per-client configuration.
- It hides state, which can surprise users.

For an alpha, this is probably one of the biggest design decisions to revisit.

### Better direction

Introduce a `HardcoverClient` or `Client` type:
...
impl HardcoverClient {
    ...
    pub async fn book(&self, id: u64) -> Result<Book, HardcoverError> {
        Book::from_id_with_client(self, id).await
    }
}
```


Even if you keep `set_api_key` for convenience, a client-based API will scale much better.

---

## 3. A new `reqwest::Client` is created for every GraphQL request
...
This defeats connection pooling and is less efficient. `reqwest::Client` is intended to be reused.

### Suggested improvement

Store a `reqwest::Client` in your own API client type, or use a static initialized client if you keep the global design.

This matters even for alpha because users may perform many lookups in a row.

---

## 4. The crate name and import name differ

Your package name is:

```toml
name = "hardcover-wrapper"
```
...
That is normal Rust behavior, but for crates.io/library ergonomics, consider whether you want the package to be named:

```toml
name = "hardcover_wrapper"
```


Many crates prefer hyphenated package names, so this is not necessarily wrong. Just make sure the README shows the import clearly.

---

## 5. `edition = "2024"` may be unnecessarily restrictive
...
If you are preparing an alpha and want broad compatibility, Rust 2021 may be safer unless you specifically need Rust 2024 features.

Rust 2024 requires a newer compiler. That is fine if intentional, but document the MSRV.

### Add to README or Cargo metadata
...
or whatever minimum version you intend to support.

---

## 6. Cargo metadata is sparse for publication

Before publishing, `Cargo.toml` should likely include:

```toml
description = "A Rust wrapper for the Hardcover GraphQL API"
license = "MIT"
repository = "https://github.com/..."
readme = "README.md"
keywords = ["hardcover", "books", "graphql", "api"]
categories = ["api-bindings"]
```


You have a `LICENSE` file, but crates.io also expects license metadata.

---

## 7. README is too minimal for alpha users

Current README only says:
...
- Feature roadmap
- How errors are currently handled, or not handled
- Link to Hardcover API docs
- License

Suggested rough structure:

```markdown
# hardcover-wrapper

A Rust API wrapper for the Hardcover GraphQL API.

## Status

This crate is in alpha. APIs may change.

## Installation
```
toml
[dependencies]
hardcover-wrapper = "0.1"
```
## Usage
```rust
...
- API schema coverage is incomplete.
- Types may change before stable release.
```
